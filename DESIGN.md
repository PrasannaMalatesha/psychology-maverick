...
> Warm storybook on cream paper — a friendly editorial canvas where oversized Bricolage Grotesque headlines and paper-cut characters share a sunlit, sticker-soft surface, with JetBrains Mono carrying the citation "receipts."

**Theme:** light

> **Source of truth.** The machine-readable system lives in `design-system/`:
> `tokens.css` (colour, the 3-font type scale with sizes + weights, spacing, radii, motion, surfaces),
> `icons.js` (the 24×24 stroke icon set), `app.css` (shell + component classes) and `shell.js` (shared rail).
> A living, rendered reference is at `design/prototype/styleguide.html`. Screens import these — they never hard-code values. The prose below documents intent; where any value here disagrees with `tokens.css`, the token file wins.

MindMarket is a warm, illustrated editorial system built on a cream-paper canvas rather than stark white, with massive Bricolage Grotesque display type that fills the frame, Inter for reading and UI, JetBrains Mono for citation receipts, and a single vivid green accent that anchors the brand across navigation strokes, borders, and hero fills. The visual language borrows from paper-cut storybook illustration — flat, vibrant character art sits directly on warm neutral backgrounds, never on photographic or gradient surfaces, and the UI chrome is deliberately minimal so the artwork leads. Components are generously rounded (50–64px radii on cards and nav), creating a soft, sticker-like quality. Color behaves decoratively rather than functionally: the green, blue, red, and yellow accents repeat across illustrations and are used sparingly in UI as borders, icon accents, and surface highlights rather than as a strict semantic state system. The overall density is breathable and confident — few elements per screen, enormous type, wide margins, and the cream canvas doing the structural work that shadow systems usually handle in product UIs.
...
A **three-family** system. Each voice has one job; they never blur into each other.

### Bricolage Grotesque — display & headings (`--font-display`)
The storybook voice. Carries authority through scale and tight tracking. Used for the hero, section ledes, and every `h1`–`h3`.
- **Weights:** 400 · 500 · 600 (default) · 700 · 800 (variable, opsz 12–96)
- **Sizes:** hero `clamp(3.4rem, 11vw, 132px)`, display ~52px, h1 40px, h2 31px, h3 24px
- **Line height:** 0.94–1.2 · **Tracking:** −0.035em at hero, tightening as size grows

### Inter — body & UI (`--font-sans`)
The reading voice. Prose, labels, inputs, buttons, nav.
- **Weights:** 400 · 500 (default UI) · 600 · 700
- **Sizes:** body-lg 18px, body 16px, ui 15px, label 14px, caption 13px, micro 12px
- **Line height:** 1.4–1.6 · **Tracking:** −0.01em on UI, normal in prose

### JetBrains Mono — the receipts (`--font-mono`)
Reserved for anything that reads as evidence: citation numbers, source codes, taxonomic/register labels, kickers. Never used for prose.
- **Weights:** 400 · 500 · 700
- **Sizes:** ~11.5px, uppercase, letter-spacing ~0.08em

### Type Scale (see `tokens.css` for the authoritative values)

| Role | Family | Size | Line Height | Tracking | Weight | Token |
|------|--------|------|-------------|----------|--------|-------|
| hero | Bricolage | clamp(3.4rem,11vw,132px) | 0.94 | -.035em | 600 | `--fs-hero` |
| display | Bricolage | clamp(2rem,5vw,52px) | 1.04 | -.025em | 600 | `--fs-display` |
| h1 | Bricolage | 40px | 1.1 | -.03em | 600 | `--fs-h1` |
| h2 | Bricolage | 31px | 1.15 | -.02em | 600 | `--fs-h2` |
| h3 | Bricolage | 24px | 1.2 | -.02em | 600 | `--fs-h3` |
| title | Inter | 20px | 1.25 | -.01em | 600 | `--fs-title` |
| lead | Inter | 19px | 1.4 | -.01em | 500 | `--fs-lead` |
| body-lg | Inter | 18px | 1.55 | — | 400 | `--fs-body-lg` |
| body | Inter | 16px | 1.6 | — | 400 | `--fs-body` |
| ui | Inter | 15px | 1.5 | — | 500 | `--fs-ui` |
| label | Inter | 14px | 1.4 | — | 500/600 | `--fs-label` |
| caption | Inter | 13px | 1.5 | — | 400 | `--fs-caption` |
| mono | JetBrains Mono | 11.5px | 1.3 | .08em ↑ | 500 | `--fs-mono` |

## Tokens — Iconography

A single stroke icon set (`design-system/icons.js`), replacing the earlier ad-hoc geometric shapes. One coherent family on a **24×24 grid, 1.8 stroke, round caps + joins**, coloured by `currentColor`. Rendered as `<svg class="ic"><use href="#ic-name"/></svg>`. Icons never carry their own colour and never mix stroke weights. (The previously hand-drawn heart and settings icons were off-grid and lopsided; both are now redrawn to correct, symmetric geometry.)
...
- Set display headlines in Bricolage Grotesque weight 600 at a fluid hero scale (up to ~132px) with tight tracking (−0.035em) and line-height ~0.94. This is the system's signature scale — stepping down to ~52px display and 40/31/24px for headings. Keep prose and UI in Inter, and reserve JetBrains Mono for citation receipts.
...
### Don't
- Don't blur the three type roles: Bricolage Grotesque for display/headings, Inter for body/UI, JetBrains Mono for citation receipts only. Never set prose in mono, and never set a heading in Inter where a Bricolage display size is called for.
